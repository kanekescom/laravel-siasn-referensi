...
use Kanekescom\Siasn\Referensi\Services\PullJenisAnakService;

class PullJenisAnakJob implements ShouldQueue
{
    ...
    {
        try {
            PullJenisAnakService::execute();

            event(new PullDataCompleted('JenisAnak', $this->user));
        } catch (\Throwable $e) {
            if ($this->user) {
                Notification::make()
