...
use Kanekescom\Siasn\Referensi\Services\PullInstansiService;

class PullInstansiJob implements ShouldQueue
{
    ...
    {
        try {
            PullInstansiService::execute();

            event(new PullDataCompleted('Instansi', $this->user));
        } catch (\Throwable $e) {
            if ($this->user) {
                Notification::make()
