...
use Kanekescom\Siasn\Referensi\Services\PullLokasiService;

class PullLokasiJob implements ShouldQueue
{
    ...
    {
        try {
            PullLokasiService::execute();

            event(new PullDataCompleted('Lokasi', $this->user));
        } catch (\Throwable $e) {
            if ($this->user) {
                Notification::make()
