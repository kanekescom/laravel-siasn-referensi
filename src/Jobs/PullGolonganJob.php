...
use Kanekescom\Siasn\Referensi\Services\PullGolonganService;

class PullGolonganJob implements ShouldQueue
{
    ...
    {
        try {
            PullGolonganService::execute();

            event(new PullDataCompleted('Golongan', $this->user));
        } catch (\Throwable $e) {
            if ($this->user) {
                Notification::make()
