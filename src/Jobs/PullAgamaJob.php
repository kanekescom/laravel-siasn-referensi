...
use Kanekescom\Siasn\Referensi\Services\PullAgamaService;

class PullAgamaJob implements ShouldQueue
{
    ...
    {
        try {
            PullAgamaService::execute();

            event(new PullDataCompleted('Agama', $this->user));
        } catch (\Throwable $e) {
            if ($this->user) {
                Notification::make()
